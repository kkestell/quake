...
void VID_Init(unsigned char *palette) {
  ...
  // Initialize display
  if (!(window = SDL_CreateWindow("Quake", SDL_WINDOWPOS_UNDEFINED,
  ...
  surface8bpp =
      SDL_CreateRGBSurface(SDL_SWSURFACE, vid.width, vid.height, 8, 0, 0, 0, 0);
  ...
  surface32bpp = SDL_CreateRGBSurface(
      surface8bpp->flags, vid.width, vid.height, 32, surface8bpp->format->Rmask,
      surface8bpp->format->Gmask, surface8bpp->format->Bmask,
      surface8bpp->format->Amask);
...
void VID_Shutdown(void) { SDL_Quit(); }
...
================
*/
void D_BeginDirectRect(int32_t x, int32_t y, uint8_t *pbitmap, int32_t width,
                       int32_t height) {
  /*
  Uint8 *offset;
  if (!screen) return;
