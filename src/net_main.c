...
void (*GetComPortConfig)(int32_t portNumber, int32_t *port, int32_t *irq,
                         int32_t *baud, bool *useModem);
void (*SetComPortConfig)(int32_t portNumber, int32_t port, int32_t irq,
                         int32_t baud, bool useModem);
void (*GetModemConfig)(int32_t portNumber, char *dialType, char *clear,
                       char *init, char *hangup);
void (*SetModemConfig)(int32_t portNumber, char *dialType, char *clear,
                       char *init, char *hangup);
  ...
  if (!configRestored) {
    if (serialAvailable) {
      if (config_com_modem.value == 1.0)
        ...
        useModem = false;
      SetComPortConfig(0, (int32_t)config_com_port.value,
                       (int32_t)config_com_irq.value,
                       (int32_t)config_com_baud.value, useModem);
...
bool IsID(struct qsockaddr *addr) {
  ...
  if ((BigLong(*(int32_t *)&addr->sa_data[2]) & 0xffffff00) == IDNET)
    return true;
  return false;
}
#endif
