...
static void NET_Listen_f(void)
{
    ...
    listening = (int32_t)strtol(Cmd_Argv(1), NULL, 0) ? true : false;
...
static void MaxPlayers_f(void)
{
    ...
    n = (int32_t)strtol(Cmd_Argv(1), NULL, 0);
    if (n < 1)
        n = 1;
...
void NET_Init(void)
{
    ...
    {
        if (i < com_argc - 1)
            DEFAULTnet_hostport = (int32_t)strtol(com_argv[i + 1], NULL, 0);
        else
    ...
    if (!configRestored)
    {
        if (serialAvailable)
        {
            if (config_com_modem.value == 1.0)
                ...
                useModem = false;
            SetComPortConfig(0, (int32_t)config_com_port.value, (int32_t)config_com_irq.value,
