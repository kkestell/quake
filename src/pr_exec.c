...
int32_t PR_EnterFunction(dfunction_t *f) {
  ...
  for (i = 0; i < c; i++)
    localstack[localstack_used + i] =
        ((int32_t *)pr_globals)[f->parm_start + i];
  localstack_used += c;
  ...
  for (i = 0; i < f->numparms; i++) {
    for (j = 0; j < f->parm_size[i]; j++) {
      ((int32_t *)pr_globals)[o] =
          ((int32_t *)pr_globals)[OFS_PARM0 + i * 3 + j];
      o++;
    }
  }
...
int32_t PR_LeaveFunction(void) {
  ...
  for (i = 0; i < c; i++)
    ((int32_t *)pr_globals)[pr_xfunction->parm_start + i] =
        localstack[localstack_used + i];
...
void PR_ExecuteProgram(func_t fnum) {
  ...
  while (1) {
    ...
    case OP_MUL_F:
      ...
      break;
    case OP_MUL_V:
      c->_float = a->vector[0] * b->vector[0] + a->vector[1] * b->vector[1] +
    ...
    case OP_CALL8:
      ...
      if (newf->first_statement <
          0) { // negative statements are built in functions
