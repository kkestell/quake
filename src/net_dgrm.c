...
int32_t Datagram_SendMessage(qsocket_t *sock, sizebuf_t *data) {
  ...
  if (sfunc.Write(sock->socket, (uint8_t *)&packetBuffer, packetLen,
                  &sock->addr) == -1)
    return -1;
...
bool Datagram_CanSendUnreliableMessage(qsocket_t *sock) { return true; }
...
int32_t Datagram_GetMessage(qsocket_t *sock) {
  ...
  while (1) {
    length = sfunc.Read(sock->socket, (uint8_t *)&packetBuffer,
    ...
    if (flags & NETFLAG_DATA) {
      ...
      sfunc.Write(sock->socket, (uint8_t *)&packetBuffer, NET_HEADERSIZE,
                  &readaddr);
...
void Datagram_Close(qsocket_t *sock) { sfunc.CloseSocket(sock->socket); }
...
static qsocket_t *_Datagram_CheckNewConnections(void) {
  ...
  if (command == CCREQ_PLAYER_INFO) {
    ...
    MSG_WriteLong(&net_message,
                  (int32_t)(net_time - client->netconnection->connecttime));
