...
int16_t ShortNoSwap(int16_t l) { return l; }
...
int32_t LongNoSwap(int32_t l) { return l; }
...
float MSG_ReadCoord(void) { return MSG_ReadShort() * (1.0 / 8); }
...
int32_t COM_FindFile(char *filename, int32_t *handle, FILE **file) {
  ...
  for (; search; search = search->next) {
    ...
    if (search->pack) {
      ...
      if (!static_registered) { // if not a registered version, don't ever go
                                // beyond base
...
uint8_t *COM_LoadHunkFile(char *path) { return COM_LoadFile(path, 1); }

uint8_t *COM_LoadTempFile(char *path) { return COM_LoadFile(path, 2); }
