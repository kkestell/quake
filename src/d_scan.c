...
void D_WarpScreen(void) {
  ...
  for (v = 0; v < scr_vrect.height + AMP2 * 2; v++) {
    rowptr[v] =
        d_viewbuffer + (r_refdef.vrect.y * screenwidth) +
        (screenwidth * (int32_t)((float)v * hratio * h / (h + AMP2 * 2)));
  }

  for (u = 0; u < scr_vrect.width + AMP2 * 2; u++) {
    column[u] =
        r_refdef.vrect.x + (int32_t)((float)u * wratio * w / (w + AMP2 * 2));
  }
...
void D_DrawTurbulent8Span(void) {
  ...
  do {
    ...
    tturb =
        ((r_turb_t + r_turb_turb[(r_turb_s >> 16) & (CYCLE - 1)]) >> 16) & 63;
...
void Turbulent8(espan_t *pspan) {
  ...
  do {
    r_turb_pdest = (unsigned char *)((uint8_t *)d_viewbuffer +
