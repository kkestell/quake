...
qpic_t *Draw_PicFromWad(char *name) { return W_GetLumpName(name); }
...
void Draw_Character(int32_t x, int32_t y, int32_t num) {
  ...
  if (r_pixbytes == 1) {
    ...
    pusdest =
        (uint16_t *)((uint8_t *)vid.conbuffer + y * vid.conrowbytes + (x << 1));
...
================
*/
void Draw_EndDisc(void) { D_EndDirectRect(vid.width - 24, 0, 24, 24); }
