...
=================
*/
void *Mod_LoadAliasFrame(void *pin, int32_t *pframeindex, int32_t numv, trivertx_t *pbboxmin, trivertx_t *pbboxmax,
...
=================
*/
void *Mod_LoadAliasGroup(void *pin, int32_t *pframeindex, int32_t numv, trivertx_t *pbboxmin, trivertx_t *pbboxmax,
                         aliashdr_t *pheader, char *name)
{
...
void *Mod_LoadAliasSkinGroup(void *pin, int32_t *pskinindex, int32_t skinsize, aliashdr_t *pheader)
{
    ...
    paliasskingroup =
        Hunk_AllocName(sizeof(maliasskingroup_t) + (numskins - 1) * sizeof(paliasskingroup->skindescs[0]), loadname);
...
void Mod_LoadAliasModel(model_t *mod, void *buffer)
{
    ...
    // skin and group info
    //
    size = sizeof(aliashdr_t) + ((pinmodel->numframes) - 1) * sizeof(pheader->frames[0]) + sizeof(mdl_t) +
            (pinmodel->numverts) * sizeof(stvert_t) +  (pinmodel->numtris) * sizeof(mtriangle_t);
    ...
    for (i = 0; i < numskins; i++)
    {
        ...
        {
            pskintype =
                (daliasskintype_t *)Mod_LoadAliasSkinGroup(pskintype + 1, &pskindesc[i].skin, skinsize, pheader);
        }
    }
    ...
    for (i = 0; i < numframes; i++)
    {
        ...
        if (frametype == ALIAS_SINGLE)
        {
            pframetype = (daliasframetype_t *)Mod_LoadAliasFrame(
                pframetype + 1, &pheader->frames[i].frame, pmodel->numverts, &pheader->frames[i].bboxmin,
                &pheader->frames[i].bboxmax, pheader, pheader->frames[i].name);
        }
        else
        {
