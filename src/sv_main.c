...
==================
*/
void SV_StartSound(edict_t *entity, int32_t channel, char *sample,
                   int32_t volume, float attenuation) {
  int32_t sound_num;
  int32_t field_mask;
  int32_t i;
  int32_t ent;
...
==================
*/
void SV_ClearDatagram(void) { SZ_Clear(&sv.datagram); }
...
void SV_WriteClientdataToMessage(edict_t *ent, sizebuf_t *msg) {
  ...
  if (val)
    ...
    items =
        (int32_t)ent->v.items | ((int32_t)pr_global_struct->serverflags << 28);
