...
void R_EmitEdge(mvertex_t *pv0, mvertex_t *pv1) {
  ...
  // it to incorrectly extend to the scan, and the extension of the line goes
  ...
  if (clip) {
    do {
      ...
      if (d0 >= 0) {
        ...
        f = d0 / (d0 - d1);
        clipvert.position[0] =
            pv0->position[0] + f * (pv1->position[0] - pv0->position[0]);
        clipvert.position[1] =
            pv0->position[1] + f * (pv1->position[1] - pv0->position[1]);
        clipvert.position[2] =
            pv0->position[2] + f * (pv1->position[2] - pv0->position[2]);
        ...
        if (d1 < 0) {
          ...
          if (!r_leftclipped)
            cacheoffset =
                FULLY_CLIPPED_CACHED | (r_framecount & FRAMECOUNT_MASK);
          return;
        }
      ...
      if (!insubmodel) {
        if (r_pedge->cachededgeoffset & FULLY_CLIPPED_CACHED) {
          ...
          if ((((uint32_t)edge_p - (uint32_t)r_edges) >
              ...
              (((edge_t *)((uint32_t)r_edges + r_pedge->cachededgeoffset))
                   ->owner == r_pedge)) {
            R_EmitCachedEdge();
            r_lastvertvalid = false;
            continue;
...
void R_RenderPoly(msurface_t *fa, int32_t clipflags) {
  ...
  while (pclip) {
    ...
    for (i = 0; i < lnumverts; i++) {
      dist =
          DotProduct(verts[vertpage][i].position, pclip->normal) - pclip->dist;
