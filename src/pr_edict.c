...
void ED_PrintNum(int32_t ent) { ED_Print(EDICT_NUM(ent)); }
    ...
    if (deathmatch.value) {
      if (((int32_t)ent->v.spawnflags & SPAWNFLAG_NOT_DEATHMATCH)) {
        ...
        continue;
                ...
                ((int32_t)ent->v.spawnflags & SPAWNFLAG_NOT_EASY)) ||
               (current_skill == 1 &&
                ((int32_t)ent->v.spawnflags & SPAWNFLAG_NOT_MEDIUM)) ||
               (current_skill >= 2 &&
                ((int32_t)ent->v.spawnflags & SPAWNFLAG_NOT_HARD))) {
      ED_Free(ent);
      inhibit++;
      continue;
    }
