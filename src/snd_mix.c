...
void S_TransferPaintBuffer(int32_t endtime) {
  ...
  */
  ...
  &pbuf, &dwSize, &pbuf2,&dwSize2, 0)) != DS_OK)
                  {
                          if (hresult != DSERR_BUFFERLOST)
                          {
                                  Con_Printf ("S_TransferPaintBuffer: DS::Lock
  Sound Buffer Failed\n"); S_Shutdown (); S_Startup (); return;
                          }

                          if (++reps > 10000)
                          {
                                  Con_Printf ("S_TransferPaintBuffer: DS:
  couldn't restore buffer\n"); S_Shutdown (); S_Startup (); return;
                          }
                  }
          }
          else
  ...
  */
  ...
  /*
          ...
          if (pDSBuf) {
                  ...
                  pDSBuf->lpVtbl->GetCurrentPosition(pDSBuf, &dwNewpos,
  &dwWrite);
