...
===================
*/
void *Hunk_Alloc(int32_t size) { return Hunk_AllocName(size, "unknown"); }

int32_t Hunk_LowMark(void) { return hunk_low_used; }
...
cache_system_t *Cache_TryAlloc(int32_t size, bool nobottom) {
  ...
  do {
    if (!nobottom || cs != cache_head.next) {
      if ((uint8_t *)cs - (uint8_t *)new >= size) { // found space
        memset(new, 0, sizeof(*new));
        new->size = size;
