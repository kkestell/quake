...
void R_AliasSetUpTransform(int32_t trivial_accept) {
  ...
  // Also scale down z, so 1/z is scaled 31 bits for free, and scale down x and
  // y correspondingly so the projected x and y come out right
...
void R_AliasTransformAndProjectFinalVerts(finalvert_t *fv, stvert_t *pstverts) {
  ...
  for (i = 0; i < r_anumverts; i++, fv++, pverts++, pstverts++) {
    ...
    zi =
        1.0 / (DotProduct(pverts->v, aliastransform[2]) + aliastransform[2][3]);
    ...
    fv->v[0] =
        ((DotProduct(pverts->v, aliastransform[0]) + aliastransform[0][3]) *
         zi) +
        aliasxcenter;
    fv->v[1] =
        ((DotProduct(pverts->v, aliastransform[1]) + aliastransform[1][3]) *
         zi) +
        aliasycenter;
...
void R_AliasPrepareUnclippedPoints(void) {
  ...
  r_affinetridesc.ptriangles =
      (mtriangle_t *)((uint8_t *)paliashdr + paliashdr->triangles);
...
void R_AliasSetupSkin(void) {
  ...
  pskindesc =
      ((maliasskindesc_t *)((uint8_t *)paliashdr + paliashdr->skindesc)) +
      skinnum;
  a_skinwidth = pmdl->skinwidth;

  if (pskindesc->type == ALIAS_SKIN_GROUP) {
    paliasskingroup =
        (maliasskingroup_t *)((uint8_t *)paliashdr + pskindesc->skin);
    pskinintervals =
        (float *)((uint8_t *)paliashdr + paliasskingroup->intervals);
    ...
    skintargettime =
        skintime - ((int32_t)(skintime / fullskininterval)) * fullskininterval;
...
void R_AliasSetupFrame(void) {
  ...
  if (paliashdr->frames[frame].type == ALIAS_SINGLE) {
    r_apverts =
        (trivertx_t *)((uint8_t *)paliashdr + paliashdr->frames[frame].frame);
    return;
  }

  paliasgroup =
      (maliasgroup_t *)((uint8_t *)paliashdr + paliashdr->frames[frame].frame);
  ...
  r_apverts =
      (trivertx_t *)((uint8_t *)paliashdr + paliasgroup->frames[i].frame);
}
...
void R_AliasDrawModel(alight_t *plighting) {
  ...
  // cache align
  pfinalverts = (finalvert_t *)(((int32_t)&finalverts[0] + CACHE_SIZE - 1) &
                                ~(CACHE_SIZE - 1));
