...
void R_ViewChanged(vrect_t *pvrect, int32_t lineadj, float aspect) {
  ...
  r_refdef.aliasvrect.height =
      (int32_t)(r_refdef.vrect.height * r_aliasuvscale);
...
void R_EdgeDrawing(void) {
  ...
  if (auxedges) {
    ...
    r_edges =
        (edge_t *)(((int32_t)&ledges[0] + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
  }

  if (r_surfsonstack) {
    surfaces =
        (surf_t *)(((int32_t)&lsurfs[0] + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
    surf_max = &surfaces[r_cnumsurfs];
    // surface 0 doesn't really exist; it's just a dummy because index 0
    // is used to indicate no edge attached to surface
    surfaces--;
...
void R_RenderView_(void) {
  ...
  // it here so that setup stuff like the refresh area calculations match what's
  // done in screen.c
  Sys_LowFPPrecision();
