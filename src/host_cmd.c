...
void Host_Status_f(void) {
  ...
  for (j = 0, client = svs.clients; j < svs.maxclients; j++, client++) {
    ...
    print("#%-2u %-16.16s  %3i  %2i:%02i:%02i\n", j + 1, client->name,
          (int32_t)client->edict->v.frags, hours, minutes, seconds);
...
void Host_Please_f(void) {
  ...
  if ((Cmd_Argc() == 3) && Q_strcmp(Cmd_Argv(1), "#") == 0) {
    ...
    if (cl->privileged) {
      cl->privileged = false;
      cl->edict->v.flags =
          (int32_t)cl->edict->v.flags & ~(FL_GODMODE | FL_NOTARGET);
...
void Host_Color_f(void) {
  ...
  if (Cmd_Argc() == 1) {
    Con_Printf("\"color\" is \"%i %i\"\n", ((int32_t)cl_color.value) >> 4,
               ((int32_t)cl_color.value) & 0x0f);
    ...
    if (hipnotic) {
      if (t[0] == '6') {
        if (t[1] == 'a')
          ...
          sv_player->v.items =
              (int32_t)sv_player->v.items | IT_GRENADE_LAUNCHER;
      } else if (t[0] == '9')
        ...
        sv_player->v.items =
            (int32_t)sv_player->v.items | (IT_SHOTGUN << (t[0] - '2'));
    } else {
      if (t[0] >= '2')
        sv_player->v.items =
            (int32_t)sv_player->v.items | (IT_SHOTGUN << (t[0] - '2'));
